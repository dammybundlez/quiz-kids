...
export default function ReviewSubmit({ playerName, questions, answers, onSubmit, onBackToQuiz }: ReviewSubmitProps) {
  ...
  return (
    <div className="min-h-screen bg-indigo-500 flex items-center justify-center p-4">
      ...
      >
        <div className="bg-white/20 backdrop-blur-xl rounded-xl p-6 md:p-8 shadow-2xl border border-white/30">
          <div className="text-center mb-6">
            <Brain size={40} className="text-yellow-300 mx-auto mb-2" />
          ...
          <div className="flex items-center justify-center gap-4 mb-5 text-sm">
            <span className="text-white font-semibold inline-flex items-center gap-1">
              <CheckCircle size={16} className="text-green-300" /> {answeredCount}/{total} answered
            </span>
            {flaggedCount > 0 && (
              <span className="text-white font-semibold inline-flex items-center gap-1">
                <Flag size={16} className="text-amber-300" /> {flaggedCount} flagged
              </span>
            )}
            {!allAnswered && (
              <span className="text-red-200 font-semibold inline-flex items-center gap-1">
                <AlertCircle size={16} /> {total - answeredCount} unanswered
              </span>
            )}
          </div>
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className="flex items-center gap-1.5 shrink-0 ml-2">
                      {ans.flagged && <Flag size={14} className="text-amber-300" />}
                      {isAnswered ? (
                        isCorrect
          ...
          <div className="flex gap-3">
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
              className="flex-1 py-3 bg-white/20 text-white font-black text-lg rounded-lg shadow-lg hover:bg-white/30 transition-all cursor-pointer tracking-wide inline-flex items-center justify-center gap-2"
            >
